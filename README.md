...
![GCP](https://img.shields.io/badge/Google_Cloud-Serverless-green.svg)
...
### Core Components
  ...
  * **Generator Node (Cloud Functions):** Acts as the "Mutation Operator," using Gemini 2.5 Flash to create variations of prompts based on parent traits (Crossover).
...
## Key Features
  ...
  * Implements a genetic algorithm cycle (Genesis -\> Evaluation -\> Selection -\> Breeding) to evolve prompts over generations.
  * Replaces brittle keyword matching with a semantic AI judge that scores outputs on nuance, tone, and accuracy, providing text-based reasoning.
  * Tests 50+ prompt variations simultaneously using Cloud Workflows' parallel loops, reducing wall-clock latency from minutes to seconds.
  * An interactive UI allows users to review the "Reasoning" of the AI Judge and manually select the best "Parent" prompts for the next generation.
  * Handles stochastic AI failures, JSON parsing errors, and safety filter collisions gracefully without crashing the pipeline.
...
1.  Open the **Streamlit UI URL** provided by the deployment command.
...
3.  Click **Launch Optimization Job**.
4.  Wait for the status to change from PENDING -\> RUNNING -\> COMPLETE.
5.  **Review Results:** Check the "Score" and "Reasoning" columns.
6.  **Evolve:** Select the best prompts using the checkboxes and click **"Spawn Generation 2"** to breed a better prompt.
...
## Engineering Challenges & Solutions
  ...
  * Initial parallelism caused cold-start exhaustion. Solved by vertically scaling workers to 2GB RAM / 1 vCPU to handle Vertex AI initialization.
  * The LLM Judge occasionally returned Markdown, breaking the pipeline. Solved via robust string cleaning and JSON parsing middleware in the worker.
  * "Pirate Talk" triggered harassment filters. Solved by explicitly configuring `SafetySettings` to BLOCK\_ONLY\_HIGH.
